...
||
코드이름 : <input type="text" name="shIfcgName" id="shIfcgName" value="<c:out value="${vo.shIfcgName }"/>">
||
...
<c:choose>
	...
	<c:otherwise>
		...
		<c:out value="${item.ifcgSeq}"/> | <a href="javascript:goForm()"><c:out value="${item.ifcgName }"/></a> | <c:out value="${item.ifcgNameEng}"/> | <c:out value="${item.ifcgDelNy}"/> <br>
